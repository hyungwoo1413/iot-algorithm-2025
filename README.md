# iot-algorithm-2025
IoT 개발자 자료구조와 알고르즘(코딩테스트) 리포지토리 2025
...
- 빅오 표기법(빠른 순)
    ...
    - $O(\log n)$ : 단순 for문, 전체 중 일부만 처리하는 것
    - $O(n)$ : 단순 for문
    - $O(n \log n)$ : 이중 for문, 안쪽 for문이 전체 중 일부만 처리
    - $O(n^2)$ : 이중 for문
    - $O(n^3)$ : 삼중 for문
    - $O(2^n)$ : 팩토리얼 등...
...
https://github.com/user-attachments/assets/43a95ed8-489f-4020-a200-88addcfc95c3
